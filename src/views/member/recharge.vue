        ...
        <div class="content">
            <div class="voucher">
                ...
                <div class="ub ub-pj ub-ac amount">
                    <p class="f20">余额</p>
                    <p class="f20">￥{{cardInfo.card.balance/100}}元</p>
                </div>
                <div class="btc1 ub ub-pj date">
                    <p class="f12">{{cardInfo.applyStoreType==='ALL'?'全店通用':'部分店铺可用'}}
                        <i class="ml10 f12">有效期：{{cardInfo.card.validity}}</i>
                    </p>
                </div>
            </div>
...
export default {
  ...
  methods: {
    submit() {
        let that = this;
        this.cardInfo.giveAmount = this.giveAmount;
        this.cardInfo.rechargeAmount = this.rechargeAmount;
  ...
  created() {
    ...
    if (this.cardInfo) {
      this.rechargeAmount = this.chargeInfo.cardInfo.rules[0].price / 100;
      this.giveAmount =
        (this.chargeInfo.cardInfo.rules[0].balance -
          this.chargeInfo.cardInfo.rules[0].price) /
        100;
    }
  }
};
...
.voucher {
  width: 100%;
  padding: 0.3rem;
  background: linear-gradient(left, #ff6c6c, #ffb444);
  color: #fff;
