            ...
            <div class="orderItem" v-for="(item,index) in orderList">
                ...
                <div class="orderInfo">
                    ...
                    <div class="ub ub-ac orderCon">
                        <ul class="orderCon-box">
                            <li>
                                ...
                                <p class="fr">
                                    <i class="icon_card mr10" v-if="item.bizType === 'MEMBER'"></i>{{item.customerName}}</p>
                            </li>
                            <li>
                    ...
                    <div class="ub ub-ac ub-pj pt10">
                        <p class="bc">实收</p>
                        <p class="bc">
                            <i class="curreny ml10">￥{{item.paidMoney? item.paidMoney/100 : 0}}</i>
                        </p>
                    </div>
...
export default {
  ...
  methods: {
    ...
    cancelOrder(id, status) {
      ...
      self.$ajax
        .get("account/merchant/module/menu/route.json", {
          params: data
        })
        .then(function(res) {
          if (res.success) {
            if (res.data.eventType === "ROUTE") {
              ...
              if (status != "已取消" && status != "已退款") {
                self.showConfirm({
                  title: "提示",
                  content:
                    "1.扣卡首款撤销卡额度恢复到扣卡前状态<br/>2.扫码首款撤销付款金额原路返回<br/>3.会员开卡撤销只可撤销未消费的会员卡",
                  leftname: "取消",
                  rightname: "确定",
                  leftListener: () => {
                    console.log("cancel");
    ...
    getData() {
      ...
      let self = this;
      this.$ajax
        .get("merchant/order/currentDayOrders.json", {
          params: data
        })
        .then(function(res) {
          if (res.success) {
...
.icon_member {
  ...
  width: 0.34rem;
  height: 0.28rem;
  background: url(../../assets/icon_member.png) no-repeat center;
